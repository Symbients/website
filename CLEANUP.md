...
**tackling cleanup LAST**, once the design (currently converging on `new_spa.html`)
is settled. Until then: note it here, keep moving.
...
- [ ] **Remove the `amber` theme site-wide** (design decision: light + dark only).
      Already removed from **`new_spa.html`** (theme token block, pre-paint
...
- [ ] **Collapse the monochrome ink tokens in `new_spa.html`.**
...
- [ ] **`assets/test-moreau-band.png` / `assets/test-moreau-hero.png`** — the
      placeholder Moreau hero band. `new_spa.html` dropped the image banner (and the
      `.hero-scrim`) entirely, so these are only referenced by `spa.html` and
...
- [ ] **Audit + optimize `assets/` (24 MB, ~199 files).** Once the page set is
      ...
      - **Optimize what's left** — several images ship as heavy single files with no
        ...
        `events/machine-consciousness.png` (~0.55 MB). The `new_spa.html` Organics cards
...
`new_spa.html` is the hybrid we are actively building. When it is blessed as the
real page, decide the fate of the scaffolding it was ported from and validated
against. Candidates for removal / retirement (do NOT delete yet — still live
reference during the port):

- [ ] `spa.html` — the rhizome scroll experiment `new_spa.html` was forked from.
...
- [ ] Decide `index.html` vs `new_spa.html` as the canonical entry, and reconcile
      the two (e.g. `new_spa.html` uses static `#sec-organics` cards **generated
      from** `registry.json`; `index.html` renders `registry.json` live via
      `registry.js` — pick one source-of-truth story).

## Housekeeping notes (verify at cleanup time)

- `new_spa.html` `#sec-organics` cards are **generated** from `registry.json`
